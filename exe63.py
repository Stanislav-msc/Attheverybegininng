...
fout=open('data63.txt', 'w')
fout.write("x\ty\tp\n")
data=[]
for y in range(floor(bmph)):
    for x in range(bmpw):
        RGB=fi.read(3)#fi.read(1),fi.read(1)
        shadow=int.from_bytes(RGB, 'little')
        if shadow<8:
            p=255-shadow#255:
            xyp=(x,y,p)
            data.append(xyp)
            ...
            fout.write(row)
    print (fi.tell(), end="\t")
    padding_bytes=fi.read(int(rs-bmpw*bpp/8))
    print (padding_bytes,fi.tell())
#print (data)
